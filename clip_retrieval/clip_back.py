        ...
        if text_input is not None and text_input != "":
            if use_mclip:
                ...
                with TEXT_PREPRO_TIME.time():
                    text = clip_resource.tokenizer([text_input]).to(
...
def get_aesthetic_embedding(model_type):
    ...
    with ThreadPool(10) as pool:
        ...
        def get(k):
            embs[k] = state_dict["linear_image.weight"].cpu().numpy()
...
def dict_to_clip_options(d, clip_options):
    return ClipOptions(
        indice_folder=(
            d["indice_folder"] if "indice_folder" in d else clip_options.indice_folder
        ),
        clip_model=d["clip_model"] if "clip_model" in d else clip_options.clip_model,
        enable_hdf5=(
            d["enable_hdf5"] if "enable_hdf5" in d else clip_options.enable_hdf5
        ),
